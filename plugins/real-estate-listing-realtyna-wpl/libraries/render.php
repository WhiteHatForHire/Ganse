...
class wpl_render
{
	...
	public static function render_price($price, $unit_id = NULL, $symbol = '', $price_text = '')
	{
        ...
        if(!trim($symbol)) $symbol = __($currency['name'], 'wpl');
		$decimal = 2;
		...
		if($currency['after_before'] == 1) $return = $price.$symbol;
		elseif($currency['after_before'] == 2) $return = $symbol.' '.$price;
		elseif($currency['after_before'] == 3) $return = $price.' '.$symbol;
