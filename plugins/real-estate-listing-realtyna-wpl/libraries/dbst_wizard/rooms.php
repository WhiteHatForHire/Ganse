...
<div class="new-rooms-wp" id="room_add">
    <select id="room_types<?php echo $field->id; ?>">
        ...
        <option value="<?php echo $room_type['id']; ?>"><?php echo __($room_type['name'], 'wpl'); ?></option>
        <?php endforeach; ?>
    </select>
    <input type="text" id="xroom<?php echo $field->id; ?>" name="xroom<?php echo $field->id; ?>" placeholder="<?php echo __('Width', 'wpl'); ?>" />
    <input type="text" id="yroom<?php echo $field->id; ?>" name="yroom<?php echo $field->id; ?>" placeholder="<?php echo __('Length', 'wpl'); ?>" />
    <button class="wpl-button button-1" onclick="wpl_save_room();"><?php echo __('Add room', 'wpl') ?></button>
</div>
<div class="rooms-list-wp" id="room_list<?php echo $field->id; ?>">
    <?php foreach($room_items as $room_item): ?>
        <div class="new-rooms room_<?php echo $room_item->id; ?>">
            ...
            <span class="room-preview"><?php echo '<span>'.__($room_item->item_name, 'wpl').'</span>'.(($room_item->item_extra1 and $room_item->item_extra2) ? '<i>'.$room_item->item_extra1.'x'.$room_item->item_extra2.'</i>' : ''); ?></span>
        </div>
    <?php endforeach; ?>
</div>
<?php
