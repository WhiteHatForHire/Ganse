...
elseif($type == 'feature' and !$done_this)
{
	...
	elseif($show == 'select')
	{
		$html .= '<label for="sf'.$widget_id.'_select_'.$field_data['table_column'].'">'.__($field['name'], 'wpl').'</label>
			<select data-placeholder="'.__($field['name'], 'wpl').'" name="sf'.$widget_id.'_select_'.$field_data['table_column'].'" id="sf'.$widget_id.'_select_'.$field_data['table_column'].'" class="wpl_search_widget_field_'.$field['id'].'_select" onchange="wpl_field_specific_changed'.$widget_id.'(\''.$field['id'].'\')" data-specific="'.$specified_children.'">
				<option value="-1" '.($current_value == -1 ? 'selected="selected"' : '').'>'.__($field['name'], 'wpl').'</option>
    ...
    elseif($show == 'options')
	{
		...
		if(!$multiple) $html .= '<option value="">'.__($field['name'], 'wpl').'</option>';
...
elseif($type == 'listings' and !$done_this)
{
	...
	if($show == 'select')
	{
		...
		if($any) $html .= '<option value="-1" selected="selected">'.__($field['name'], 'wpl').'</option>';
	...
	{
		$i = 0;
		if($any) $html .= '<input name="rdo'.$widget_id.'_select_'.$field_data['table_column'].'" type="radio" value="-1" id="rdo'.$widget_id.'_select_'.$field_data['table_column'].'_'.$i.'" onclick="wpl_select_radio'.$widget_id.'(this.value, this.checked, \''.$field_data['table_column'].'\');" onchange="wpl_listing_changed'.$widget_id.'(this.value);"><label for="rdo'.$widget_id.'_select_'.$field_data['table_column'].'_'.$i.'">'.__($field['name'], 'wpl').'</label>';
	...
	elseif($show == 'select-predefined')
	{
		...
		foreach($listings as $listing)
		{
			if(is_array($field['extoption']) and in_array($listing['id'], $field['extoption'])) $html .= '<option value="'.$listing['id'].'" '.($current_value == $listing['id'] ? 'selected="selected"' : '').'>'.__($listing['name'], 'wpl').'</option>';
		}
...
elseif($type == 'number' and !$done_this)
{
    ...
    elseif($show == 'minmax')
    {
		...
		$html .= '<input name="sf'.$widget_id.'_tmin_'.$field_data['table_column'].'" type="text" id="sf'.$widget_id.'_tmin_'.$field_data['table_column'].'" value="'.(trim(wpl_request::getVar('sf_tmin_'.$field_data['table_column'])) != '' ? $current_min_value : '').'" placeholder="'.__('Min', 'wpl').'" />';
        
		$html .= '<label class="wpl_search_widget_to_label" for="sf'.$widget_id.'_tmax_'.$field_data['table_column'].'">'.__('Max', 'wpl').'</label>';
		$html .= '<input name="sf'.$widget_id.'_tmax_'.$field_data['table_column'].'" type="text" id="sf'.$widget_id.'_tmax_'.$field_data['table_column'].'" value="'.(trim(wpl_request::getVar('sf_tmax_'.$field_data['table_column'])) != '' ? $current_max_value : '').'" placeholder="'.__('Max', 'wpl').'" />';
	}
...
elseif($type == 'property_types' and !$done_this)
{
	...
	{
		$i = 0;
		if($any) $html .= '<input name="rdo'.$widget_id.'_select_'.$field_data['table_column'].'" type="radio" value="-1" id="rdo'.$widget_id.'_select_'.$field_data['table_column'].'_'.$i.'" onclick="wpl_select_radio'.$widget_id.'(this.value, this.checked, \''.$field_data['table_column'].'\');"><label for="rdo'.$widget_id.'_select_'.$field_data['table_column'].'_'.$i.'">'.__($field['name'], 'wpl').'</label>';
	...
	elseif($show == 'select-predefined')
	{
		...
		foreach($property_types as $property_type)
		{
			if(is_array($field['extoption']) and in_array($property_type['id'], $field['extoption'])) $html .= '<option class="wpl_pt_parent wpl_pt_parent'.$property_type['parent'].'" value="'.$property_type['id'].'" '.($current_value == $property_type['id'] ? 'selected="selected"' : '').'>'.__($property_type['name'], 'wpl').'</option>';
		}
...
elseif(in_array($type, array('select', 'multiselect')) and !$done_this)
{
	...
	if($show == 'select')
	{
		$html .= '<select data-placeholder="'.__($field['name'], 'wpl').'" name="sf'.$widget_id.'_'.$type.'_'.$field_data['table_column'].'" class="wpl_search_widget_field_'.$field['id'].'" id="sf'.$widget_id.'_'.$type.'_'.$field_data['table_column'].'">';
		if($any) $html .= '<option value="-1">'.__($field['name'], 'wpl').'</option>';
		...
		foreach($options['params'] as $option)
		{
			...
			$html .= '<option value="'.$option['key'].'" '.($current_value == $option['key'] ? 'selected="selected"' : '').'>'.stripslashes(__($option['value']), 'wpl').'</option>';
		}
	...
	elseif($show == 'multiple')
    {
        ...
        $html .= '<div class="wpl_searchwid_'.$field_data['table_column'].'_multiselect_container">
		<select data-placeholder="'.__($field['name'], 'wpl').'" class="wpl_searchmod_'.$field_data['table_column'].'_multiselect" id="sf'.$widget_id.'_'.$query_type.'_'.$field_data['table_column'].'" name="sf'.$widget_id.'_'.$query_type.'_'.$field_data['table_column'].'" multiple="multiple">';
		
        foreach($options['params'] as $option)
		{
			if(!$option['enabled']) continue;

			$html .= '<option value="'.$option['key'].'" '.(in_array($option['key'], $current_values) ? 'selected="selected"' : '').'>'.stripslashes(__($option['value'], 'wpl')).'</option>';
        }
...
elseif(($type == 'price' or $type == 'mmprice') and !$done_this)
{
	...
	if($show == 'minmax')
	{
        ...
        if(trim(wpl_request::getVar('sf_min_'.$min_column)) == '') $current_min_value = '';
        if(trim(wpl_request::getVar('sf_max_'.$max_column)) == '') $current_max_value = '';
        
		if($input_type == 'text') $html .= '<label id="wpl_search_widget_from_label'.$widget_id.'" class="wpl_search_widget_from_label" for="sf'.$widget_id.'_min_'.$min_column.'">'.sprintf(__('Min %s', 'wpl'), $field_data['name']).'</label>';
		$html .= '<input name="sf'.$widget_id.'_min_'.$min_column.'" type="'.$input_type.'" id="sf'.$widget_id.'_min_'.$min_column.'" value="'.$current_min_value.'" placeholder="'.sprintf(__('Min %s', 'wpl'), $field_data['name']).'" />';
        
		if($input_type == 'text') $html .= '<label id="wpl_search_widget_to_label'.$widget_id.'" class="wpl_search_widget_to_label" for="sf'.$widget_id.'_max_'.$max_column.'">'.sprintf(__('Max %s', 'wpl'), $field_data['name']).'</label>';
		$html .= '<input name="sf'.$widget_id.'_max_'.$max_column.'" type="'.$input_type.'" id="sf'.$widget_id.'_max_'.$max_column.'" value="'.$current_max_value.'" placeholder="'.sprintf(__('Max %s', 'wpl'), $field_data['name']).'" />';
	}
	elseif($show == 'minmax_slider')
	{
...
elseif(($type == 'area' or $type == 'volume' or $type == 'length') and !$done_this)
{
	...
	if($show == 'minmax')
	{
		...
		$html .= '<input name="sf'.$widget_id.'_min_'.$field_data['table_column'].'" type="'.$input_type.'" id="sf'.$widget_id.'_min_'.$field_data['table_column'].'" value="'.(trim(wpl_request::getVar('sf_min_'.$field_data['table_column'])) != '' ? $current_min_value : '').'" placeholder="'.__('Min', 'wpl').'" />';
		...
		$html .= '<input name="sf'.$widget_id.'_max_'.$field_data['table_column'].'" type="'.$input_type.'" id="sf'.$widget_id.'_max_'.$field_data['table_column'].'" value="'.(trim(wpl_request::getVar('sf_max_'.$field_data['table_column'])) != '' ? $current_max_value : '').'" placeholder="'.__('Max', 'wpl').'" />';
	}
	elseif($show == 'minmax_slider')
	{
        wpl_html::set_footer('<script type="text/javascript">
