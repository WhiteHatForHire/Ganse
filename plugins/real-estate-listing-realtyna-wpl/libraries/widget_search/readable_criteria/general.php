...
{
    if($value != '-1' and trim($value) != '') $readable_search_field[$table_column] = sprintf(__('Equal or more than %s', 'wpl'), $value);

	$done_this = true;
}
elseif($format == 'tmax' and !$done_this)
{
    if($value != '-1' and trim($value) != '') $readable_search_field[$field_name] = sprintf(__('Equal or less than %s', 'wpl'),$value);
	$done_this = true;
}
elseif($format == 'multiple' and !$done_this)
{
    if(!($value == '' or $value == '-1' or $value == ','))
	{
		...
		if($value != '')
        {
            ...
            $readable_search_field[$field_name] = sprintf(__('In these values: %s', 'wpl'), $generated_values);
        }
	}
...
{
    if(trim($value) != '') $readable_search_field[$field_name] = sprintf(__('Contains %s', 'wpl'), $value);
	$done_this = true;
}
elseif($format == 'between' and !$done_this)
{
    if($value != '-1' and trim($value) != '')
    {
        ...
        $readable_search_field[$field_name] = sprintf(__('Between %s and %s', 'wpl'), $min, $max);
    }
...
elseif($format == 'betweenunit' and !$done_this)
{
    if($value != '-1' and trim($value) != '')
	{
        ...
        $string = __('Between ', 'wpl');
        if($si_value_min != 0) $string .= $si_value_min;
		if($si_value_max != 0) $string .= sprintf(__(' and %s', 'wpl'), $si_value_max);
...
elseif($format == 'datemin' and !$done_this)
{
	if(trim($value) != '')
	{
		...
		$readable_search_field[$field_name] = sprintf(__('Equal or more than %s', 'wpl'), $value);
	}
...
elseif($format == 'datemax' and !$done_this)
{
	...
	{
		$value = wpl_render::derender_date($value);
		$readable_search_field[$field_name] = sprintf(__('Equal or less than %s', 'wpl'), $value);
	}
...
{
	if(trim($value) != '') $readable_search_field[$field_name] = sprintf(__('Equal or more than %s', 'wpl'), $value);
	$done_this = true;
}
elseif($format == 'rawdatemax' and !$done_this)
{
	if(trim($value) != '') $readable_search_field[$field_name] = sprintf(__('Equal or less than %s', 'wpl'), $value);
	$done_this = true;
}
elseif($format == 'notselect' and !$done_this)
{
    $readable_search_field[$field_name] = sprintf(__('Not select in %s', 'wpl'), $value);
	$done_this = true;
}
elseif($format == 'parent' and !$done_this)
{
	if($value != '-1' and trim($value) != '')
	{
		...
		$readable_search_field[$field_name] = sprintf(__('%s As the Parent field', 'wpl'), $value);
	}
...
{
    $readable_search_field[$field_name] = sprintf(__('Contains %s', 'wpl'), $value);
	$done_this = true;
}
elseif($format == 'unit' and !$done_this)
{
	if($value != '-1' and trim($value) != '')
	{
        ...
        if(!empty($si_value_max) and (empty($si_value_min) or $si_value_min == 0)) $string = __('Up to ', 'wpl');
        elseif(!empty($si_value_min) and (empty($si_value_max) or $si_value_max == 0)) $string = __('More than ', 'wpl');
        elseif(!empty($si_value_min) and !empty($si_value_max)) $string = __('Between ', 'wpl');
        else $string = '';

        if($si_value_min != '') $string .= $si_value_min.$unit_data['name'];
		if($si_value_max != '') $string .= sprintf(__(' and %s','wpl'), $si_value_max.$unit_data['name']);
...
{
    $readable_search_field['Location Text'] = $value;
	$done_this = true;
}
