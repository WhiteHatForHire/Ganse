...
elseif($type == 'select' and !$done_this) //////////////////////////// select ////////////////////////////
{
	if($value != '' and $value != '-1')
	{
		...
		{
			if($value == $field_option['key']) $return['value'] = stripslashes(__($field_option['value'], 'wpl'));
		}
	...
	if($values[$field->table_column] != 0) 
	{
		...
		if(isset($options['values']))
		{
			foreach($options['values'] as $field_option)
			{
				...
				{
					$return['values'][$i] = stripslashes(__($field_option['value'], 'wpl'));
					$i++;
				}
			}
		}
...
elseif($type == 'multiselect' and !$done_this) //////////////////////////// multiselect ////////////////////////////
{
	if($value != '' and $value != '-1')
	{
		...
		foreach($options['params'] as $field_option)
		{
			if(in_array($field_option['key'], $keys)) $multiselect_values[] = stripslashes(__($field_option['value'], 'wpl'));
		}
