...
class wpl_property
{
	...
	public static function finalize($property_id, $mode = 'edit', $user_id = NULL)
	{
		...
		$update_query .= "`finalized`='1', `mls_id_num`=cast(`mls_id` AS unsigned), ";
		if(wpl_global::check_access('confirm', $user_id)) $update_query .= "`confirmed`='1',";
		
		$query = "UPDATE `#__wpl_properties` SET ".trim($update_query, ', ')." WHERE `id`='$property_id'";
		...
		foreach($rendered as $data)
		{
			...
			elseif($type == 'locations' and isset($data['locations']) and is_array($data['locations']))
			{
				...
				foreach($data['locations'] as $location_level=>$value)
				{
                    ...
                    if($name !== $abbr) array_push($location_values, stripslashes_deep($abbr));
				}
	...
	public static function generate_location_text($property_data, $property_id = 0, $glue = ',', $force = false, $only_return = false)
	{
        ...
        if(isset($property_data['show_address']) and !$property_data['show_address'])
        {
            ...
            return __($location_hidden_keyword, 'wpl');
        }
        ...
        if(isset($property_data[$street_suffix_column]) and trim($property_data[$street_suffix_column]) != '') $locations['street_suffix'] = __($property_data[$street_suffix_column], 'wpl');
        ...
        // Location Abbr Names
        if(isset($property_data['location1_name']) and trim($property_data['location1_name'])) $locations['location1_abbr'] = __(wpl_locations::get_location_abbr_by_name($property_data['location1_name'], 1), 'wpl');
        if(isset($property_data['location2_name']) and trim($property_data['location2_name'])) $locations['location2_abbr'] = __(wpl_locations::get_location_abbr_by_name($property_data['location2_name'], 2), 'wpl');
	...
	public static function update_alias($property_data, $property_id = 0, $glue = '-', $force = false, $only_return = false)
	{
        ...
        // Location Abbr Names
        if(isset($property_data['location1_name']) and trim($property_data['location1_name'])) $alias['location1_abbr'] = __(wpl_locations::get_location_abbr_by_name($property_data['location1_name'], 1), 'wpl');
        if(isset($property_data['location2_name']) and trim($property_data['location2_name'])) $alias['location2_abbr'] = __(wpl_locations::get_location_abbr_by_name($property_data['location2_name'], 2), 'wpl');
        
		$alias['location'] = self::generate_location_text($property_data, $property_id, '-', false, $only_return);
	...
	public static function property_visited($property_id)
	{
        ...
        if(wpl_global::is_multisite() and (!$current_user_id or ($current_user_id and $current_user_id != wpl_property::get_property_user($property_id))))
        {
            ...
            $query = "SELECT `id` FROM `#__wpl_property_visits` WHERE `pid`='$property_id' AND `blog_id`='$blog_id' LIMIT 1";
            $result = wpl_db::select($query, 'loadResult');

            if($result)
            {
                $query = "UPDATE `#__wpl_property_visits` SET `visits`=visits+1 WHERE `pid`='$property_id' AND `blog_id`='$blog_id'";
                wpl_db::q($query, 'update');
            }
            else
            {
                $query = "INSERT INTO `#__wpl_property_visits` (`pid`, `visits`, `blog_id`) VALUES ('$property_id', 1, '$blog_id')";
	...
	public static function import($properties_to_import, $wpl_unique_field = 'mls_id', $user_id = NULL, $source = 'mls', $finalize = true, $log_params = array())
	{
		...
		foreach($properties_to_import as $property_to_import)
		{
			...
			if(!$exists) $pid = $model->create_property_default($user_id, $kind);
    ...
    public static function location_visibility($property_id, $kind = NULL, $membership_id = NULL)
    {
        ...
        if(!in_array($membership_id, $accesses)) return __($dbst['accesses_message'], 'wpl');
