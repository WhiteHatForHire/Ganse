	...
	if(!($value == '' or $value == '-1' or $value == ','))
	{
		...
		if($value != '')
        {
            ...
            foreach($values_ex as $value_ex)
            {
            	...
            	$q .= "(`".$table_column."` = '".$value_ex."' OR `".$table_column."` LIKE '".$value_ex.",%' OR `".$table_column."` LIKE '%,".$value_ex."' OR `".$table_column."` LIKE '%,".$value_ex.",%') AND ";
            }
...
elseif($format == 'mmunit' and !$done_this)
{
	if($value != '-1' and trim($value) != '')
	{
        ...
        $min = (isset($vars['sf_min_'.$table_column]) and $vars['sf_min_'.$table_column] != '-1') ? $vars['sf_min_'.$table_column] : 0;
		$max = (isset($vars['sf_max_'.$table_column.'_max']) and $vars['sf_max_'.$table_column.'_max'] != '-1') ? $vars['sf_max_'.$table_column.'_max'] : 0;
