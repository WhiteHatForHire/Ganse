...
class wpl_flex
{
     ...
     * @return object|null
     */
    public static function get_field_by_column($column, $kind)
	{
        /** return from cache if exists **/
        if(isset(self::$fields_by_column[$kind]))
        {
            if(isset(self::$fields_by_column[$kind][$column])) return self::$fields_by_column[$kind][$column];
            else
            {
        ...
        $query = "SELECT * FROM `#__wpl_dbst` WHERE 1";
        $results = wpl_db::select($query, 'loadObjectList');

        $fields = array();
        foreach($results as $result)
        {
            if(!isset($fields[$result->kind])) $fields[$result->kind] = array();
            if(!trim($result->table_column)) continue;
            
            $fields[$result->kind][$result->table_column] = $result;
        }

        /** add to cache **/
        self::$fields_by_column = $fields;
