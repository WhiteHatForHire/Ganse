...
foreach($wpl_properties as $key=>$gallery)
{
    ...
    <div class="wpl-carousel-item" '.$this->itemscope.' '.$this->itemtype_SingleFamilyResidence.'>
        <img class="lazyOwl" '.$this->itemprop_image.' src="'.$image_url.'" data-src="'.$image_url.'" alt="'.$image_alt.'" height="'.$image_height.'" style="height: '.$image_height.'px;" />
        <div class="title">
            <h3 '.$this->itemprop_name.'>'.$image_title.'</h3>
            <h4 '.$this->itemprop_address.' style="display:none">'.$image_location.'</h4>
            <a '.$this->itemprop_url.' class="more_info" href="'.$gallery["property_link"].'">'. __('More', 'wpl').'</a>
        </div>
		'.$tags_str.'
    </div>';
}
