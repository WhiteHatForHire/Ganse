...
class wpl_global
{
	...
	public static function min_access($role = 'guest', $user_id = NULL)
	{
		...
		{
			echo __("Sorry, you currently don't have access to this page!", 'wpl');
			exit;
		}
	}
	...
	public static function check_addon_update($addon_id)
	{
		...
		if(!$addon_data->updatable) return array('success'=>0, 'message'=>__('Error: #U201, An add-on update is not available for this domain or update key!', 'wpl'));
		...
		$message = $answer['update'] ? __('Updating ... please wait', 'wpl') : __('Your add-on is up to date!', 'wpl');
    ...
    public static function render_pattern($the_pattern, $property_id, $property_data, $glue = ',', $data = NULL)
    {
        ...
        if(is_null($data))
        {
            ...
            // Location Abbr Names
            if(isset($property_data['location1_name']) and trim($property_data['location1_name'])) $data['location1_abbr'] = __(wpl_locations::get_location_abbr_by_name($property_data['location1_name'], 1), 'wpl');
            if(isset($property_data['location2_name']) and trim($property_data['location2_name'])) $data['location2_abbr'] = __(wpl_locations::get_location_abbr_by_name($property_data['location2_name'], 2), 'wpl');
    ...
    public static function verify_google_recaptcha($response, $option)
    {
        ...
        if($enabled === '1' && $gre_section === '1')
        {
            ...
            if($response === NULL || $response === '' || $response === 'undefined') return array('success'=>0, 'message'=>__('Please set recaptcha.', 'wpl'));
            elseif(!wpl_global::get_recaptcha_response($response)) return array('success'=>0, 'message'=>__('Captcha is invalid! Please try again.', 'wpl'));
    ...
    */
    public static function generate_readable_criteria($search_fields, $needle_str = 'sf_')
    {
        ...
        foreach ($search_fields as $field=>$value)
        {
            ...
            $format = $ex[1];
			$table_column = str_replace($needle_str.$format.'_', '', $field);
            ...
            $field_name = empty($field_name) ? $table_column : $field_name;
            $done_this = false;
    ...
    */
    public static function generate_readable_criteria_values($table_column, $value)
    {
        ...
        {
            $zero_one = array('0'=>__('Disabled', 'wpl'), '1'=>__('Enabled', 'wpl'));
