...
class wpl_functions_controller extends wpl_controller
{
    ...
    private function report_abuse_submit()
	{
        ...
        else
        {
            ...
            if($PRO->report_abuse_send($parameters))
            {
                ...
                $returnData['message'] = __('Report sent successfully.', 'wpl');
            }
            else
            {
                $returnData['success'] = 0;
    ...
    {
        // Check Nonce
        if(!wpl_security::verify_nonce(wpl_request::getVar('_wpnonce', ''), 'wpl_watch_changes_form')) $this->response(array('success'=>0, 'message'=>__('The security nonce is not valid!', 'wpl')));
        ...
        else
        {
            ...
            if(!$user_id)
            {
                if($parameters['guest_method'] == 'register')
                {
                    ...
                    {
                        $returnData['success'] = 0;
                        $returnData['message'] = __('You must fill all the fields!', 'wpl');
                        echo json_encode($returnData);
                        exit;
                    }
                    ...
                    {
                        $returnData['success'] = 0;
                        $returnData['message'] = __('Invalid email!', 'wpl');
                        echo json_encode($returnData);
                        exit;
                    }
                    ...
                    {
                        $returnData['success'] = 0;
                        $returnData['message'] = __('Email exists!', 'wpl');
                        echo json_encode($returnData);
                        exit;
                    }
            ...
            $returnData['success'] = 1;
            $returnData['message'] = __('Saved successfully.', 'wpl');
        }
