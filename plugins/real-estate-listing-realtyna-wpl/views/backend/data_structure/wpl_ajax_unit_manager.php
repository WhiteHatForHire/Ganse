...
class wpl_data_structure_controller extends wpl_controller
{
	...
	**/
	private function check_eligibility($unit_id, $enabled)
	{
		...
		if((int) $count === 1 && (int) $enabled === 0) wp_send_json(array('success'=>0, 'message'=>__('You should have one active unit.', 'wpl'), 'data'=>''));
	...
	**/
	private function replaceunit_with_activeunit()
	{
		...
		// Check for response
		if($response > 0) wp_send_json(array('success'=> 1, 'message'=> __('Listings changed successfully.', 'wpl'), 'data'=>$response));
		else wp_send_json(array('success'=> 0, 'message'=> __('No listing found.', 'wpl'), 'data'=>''));
	}	
}