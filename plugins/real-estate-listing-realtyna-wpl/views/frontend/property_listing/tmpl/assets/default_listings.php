...
foreach($this->wpl_properties as $key=>$property)
{
	...
	$parking = (isset($property['materials']['f_150']['values'][0]) and trim($property['materials']['f_150']['values'][0])) ? sprintf('<div class="parking"><span class="value">%s</span><span class="name">%s</span></div>', $property['materials']['f_150']['values'][0], __("Parking(s)", "wpl")) : '';
			...
			<div class="wpl_prp_bot">
				...
				<div class="wpl_prp_listing_icon_box"><?php echo $room . $bathroom . $parking . $pic_count . $build_up_area; ?>
					...
					<div class="visits_box">
						<span class="name"><?php echo __('Visits', 'wpl'); ?>:</span><span class="value"><?php echo $property['data']['visit_time']; ?></span>
					</div>
                    <?php endif; ?>
				</div>
					...
					<div class="wpl_listing_links_container">
						<ul>
							...
							<li class="favorite_link<?php echo ($find_favorite_item ? ' added' : '') ?>">
								<a href="#" style="<?php echo ($find_favorite_item ? 'display: none;' : '') ?>" id="wpl_favorite_add_<?php echo $property_id; ?>" onclick="return wpl_favorite_control('<?php echo $property_id; ?>', 1);" title="<?php echo __('Add to favorites', 'wpl'); ?>"></a>
								<a href="#" style="<?php echo (!$find_favorite_item ? 'display: none;' : '') ?>" id="wpl_favorite_remove_<?php echo $property_id; ?>" onclick="return wpl_favorite_control('<?php echo $property_id; ?>', 0);" title="<?php echo __('Remove from favorites', 'wpl'); ?>"></a>
							</li>
						</ul>
					</div>
				</div>
